...
redelivers OAR records to observers. Native APIs may use these words differently.
Mutable message rows can be resume material even without an append-only frame log;
report untested context restoration as unverified rather than absent.
...
reproducible from this directory alone and do not establish support guarantees.
