...
Conclusion for `/sync/replay`: **the client supplies the restored sessionID
inside the event.** This does not establish how normal session creation mints IDs. `?workspace=` only sets `event_sequence.owner_id`; the
