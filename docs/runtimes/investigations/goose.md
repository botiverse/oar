...
model turn was completed), so frame-level granularity is unmeasured. No append-only frame log was found in the inspected storage; this is not
a proof about untested storage paths or full model turns.
...
The server then mints two identities: `acp-connection-id` (a UUID) and
`sessionId` (a `YYYYMMDD_N` counter). The secret is a credential, not a
server-minted identity. The database has **no owner column at
...
- **Multiple observers: yes, but as separate independent streams.** Two
  ...
  **B receives `current_mode_update` and the still-connected A receives zero
  ...
  events are per-connection, and no cross-connection mode update was observed in that window.**
...
| runtime side resume material | mutable relational rows; model-context restoration was not tested because no model turn ran |
| broadcast and subscription separated | not separated, and weaker than that: no mode-change broadcast observed; other event families untested |

Absence of an append-only frame log does not imply absence of resume material.
`session/load` restored session settings from mutable rows. Whether it restores
model context after actual turns remains unverified in this probe.
...
- **Shared mutable state with per-connection events is the weakest multi-
  ...
  in state but not in events, because no mode-change broadcast was observed. A host that wants
