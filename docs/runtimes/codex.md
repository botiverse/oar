...
projection, transport, or test code at the investigation baseline, with current mapping notes below; **source upstream** is
...
by @Faye** is her app-server probe of 2026-09-12 ([archived investigation PR][faye-probe]), run over the websocket transport against a local
Responses API mock with an isolated `CODEX_HOME`, so it speaks to server
behaviour, not to OAR's stdio path (death boundary results in the same [investigation PR][faye-death]); **vendor** is native documentation that
...
1. **Entry.** `codex app-server -c sandbox_mode="danger-full-access"
   ...
   [adapter][oar-session]; vendor README.
...
3. **Event model.** Every JSON-RPC notification is one event record whose
   ...
   reason. Source: [projection][oar-projection] and 150 to
...
4. **Ownership and identity.** The spawning OAR process owns the child.
   ...
   not verifiable (observed by @Faye). Source: adapter and 154,
   projection.
5. **Capability honesty.** Declared `{ steer: true, queue: { durable: true },
  ...
  ; [open gaps](#verification-and-open-gaps).
...
8. **Extension points.** MCP servers, skills, apps, collab agents, and
   ...
   and the model list. Source: adapter; vendor README.
...
1. **Is the native session id stable across a host restart, and can it be
   ...
   Reopening needs the rollout under the active `CODEX_HOME`, and a completed turn is not required if the rollout was already written.
   A newly allocated id without a rollout is not enough to resume. Resuming on a connection already
...
4. **Can a second observer attach to the same session?** On OAR's stream,
   yes and without limit: `rawEvents` with a cursor replays retained
   records after `afterSeq`, then continues live (source, kernel
...
6. **Do hosted forms report environment lifecycle events?** Not on OAR's
   ...
   @Faye), and whether a client is told about an unload remains unverified; the
   daemon and proxy paths are still open.

### Tool call outcome reporting

Codex reports status on the tool item types listed below, and OAR already holds it in
...
`item-detail.ts` provides an output summary. The current projection also maps
`status: failed` to `tool_call_ended.result: failed` and `completed` to `ok`.
Other statuses, including `declined`, leave the normalized result unspecified;
the native status remains on the frame. A `webSearch` item has no status.

Current input identity is documented in [Inputs and conversation projection](../spec/conversation.md):
OAR sends `clientUserMessageId` for prompt, steer, and queue, then correlates
`item/started` user messages through `item.clientId`. Acceptance and native
observation remain separate from model consumption.
...
so the exact live notification sequence, especially deltas, cannot be recovered
from the runtime. Persisted items remain available through history APIs.
...
[faye-death]: https://github.com/botiverse/oar/pull/17
