...
evidence: **source** is OAR adapter, projection, or test code at the investigation baseline, with current mapping notes below; **observed** is a recorded run or a file inspected on a named
...
5. **Capability honesty.** Declared `{ steer: true, queue: { durable: false },
   ...
   received by the model. Queue is adapter held. Source: adapter;
...
8. **Extension points.** MCP, agents, skills, plugins, hooks, and permission
   ...
   `list_models` remains an experimental probe, not the discovery path.
...
4. **Can a second observer attach to the same session?** On OAR's stream,
   yes and without limit: `rawEvents` with a cursor replays retained records
   after `afterSeq`, then continues live (source, kernel).
...
2217 `false` and 158 `true`. OAR records the block verbatim in `native` and now maps `is_error` in
`toolResultViews` to `tool_call_ended.result`: `true` becomes `failed`,
`false` becomes `ok`, and an absent field leaves the result unspecified.
A missing tool result after process death is not an observed failure.
