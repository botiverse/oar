...
| `pi-list-models.ts` | `pi --list-models` = live ModelRegistry (extension-registered, credential-scoped, volatile); `pi auth check` is typed but differently scoped; in-process `getAvailableSnapshot()` is empty until `getAvailable()` runs (the `oar models pi` "no models" bug) | 2026-09-05, pi (local install) + SDK 0.84.2 |
