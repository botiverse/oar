...
import { awaitTurnEnd, codexRuntime, type RawEvent } from "../packages/oar/src/index.js";
...
session.rawEvents((record) => {
  ...
  const label = record.kind === "frame"
    ? `${record.body.type} ${record.body.events.map((view) => view.kind).join(",")}`
...
const textAfter = (seq: number): string => records
  .filter((record) => record.seq > seq && record.kind === "frame")
